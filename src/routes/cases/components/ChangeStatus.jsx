...
const ChangeStatus = ({ id }) => {
  ...
  const [workStartDatetime, setWorkStartDatetime] = useState(null);
  const [workEndDatetime, setWorkEndDatetime] = useState(null);
  ...
  const [caseUserStatus, setCaseUserStatus] = useState({});
  ...
  const getStatusWorkload = async (casee) => {
    // Get the latest status history from case status users 
    if (casee?.case_status_users?.length > 0) {
      const filtere_status = casee.case_status_users.filter(item => item.case_status === casee.status);
      const latestEntry = filtere_status.reduce((latest, item) => {
        return !latest || new Date(item.created_at) > new Date(latest.created_at) ? item : latest;
      }, null);
      // console.log(latestEntry);

      if (latestEntry) {
        setCaseUserStatus(latestEntry);
      }

      if (latestEntry?.work_start_datetime) {
        setWorkStartDatetime(latestEntry.work_start_datetime);
      }

      if (latestEntry?.work_end_datetime) {
        setWorkEndDatetime(latestEntry.work_end_datetime);
      }

      console.log('now here');
      console.log(workEndDatetime);
      console.log(workStartDatetime);
    }
  }
  ...
  const handle_workload = async (way) => {
    ...
    // console.log(latestEntry);
    if (latestEntry) {
      const payload = latestEntry;
      if (way == 'start') {
        payload.work_start_datetime = formattedDate;
      } else {
        payload.work_end_datetime = formattedDate;
      }

      // console.log(payload);
      var result = await CaseService.updateCaseWorkload(payload);
      console.log(result?.data)
      if (result?.data?.status == 200) {
        setErrorMsg(result?.data?.message);

        if (way == 'start') {
          setWorkStartDatetime(formattedDate);
        } else {
          setWorkEndDatetime(formattedDate);
        }
      } else {
        setErrorMsg('Staus Workload Not Updated!');
      }
    }
  ...
  return (
    <div className="users-wrapper patinet-info">
      ...
      <div className="change-status-wrapper">
        {(casee?.status == "9" || casee?.status == "10" || casee?.status == "11" || casee?.status == "12" || casee?.status == "13" || casee?.status == "14" || casee?.status == "15" || casee?.status == "16" || casee?.status == "17") &&
            ...
            <div className="col-12 col-md-8">
              ...
              {casee?.planner ?
                <div>
                  ...
                  {(casee?.status == '11' || casee?.status == '12' || casee?.status == '16') ? <div className="message-div">Files are uploaded for Client check</div> : <div className="form-group  mb-4">
                ...
                :
                <div className="message-div">Case is not assigned yet to any planner</div>
              }
            </div>
          </div>
        ) : (
          (casee?.status !== '1' && casee?.status !== '18') && <div className="row">
            <div className="col-12 col-md-8">
              ...
              {casee?.status == '18' ? <div className="message-div">Case is completed</div> : <div className="form-group  mb-4">
                <label htmlFor="">Select case status </label>
                {casee?.planner ? <select
                  className="input"
                  value={caseStatus}
                  onChange={(e) => setCaseStatus(e.target.value)}
                  disabled={workEndDatetime? false : true}
                ...
                </select> : <div className="message-div">Case is not assigned yet to any planner</div>}
                {(userType == 'client' || userType == 'sub_client') && (casee?.status == '3' || casee?.status == '5' || casee?.status == '6' || casee?.status == '8' || casee?.status == '9' || casee?.status == '10' || casee?.status == '15' || casee?.status == '13' || casee?.status == '14' || casee?.status == '17') ? <div className="message-div">Case is under process</div> : ''}
                {userType == 'treatment_planner' && <div> {casee?.status == '3' ? <div className="message-div"> Case QC Department </div> : casee?.status == '4' ?
                  <div className="message-div">Case pending for need more info from client</div>
                  : ''} </div>}
                {caseStatus && (
                  <div className=" mb-4">
                    <div className="form-group">
        ...
        {casee?.status != "1" && !workStartDatetime &&
          <button onClick={() => handle_workload('start')} className="button">Start Work</button>
        }
        {(casee?.status != "1" && workStartDatetime && !workEndDatetime) &&
          <button onClick={() => handle_workload('end')} className="button ms-4">Complete Work</button>
        }
        {errorMsg && workStartDatetime && workEndDatetime &&
        <p>{errorMsg}</p>}
